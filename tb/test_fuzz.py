...
# Fires hundreds of valid, corrupted, and random frames and asserts the
# hardware verdict matches the specification exactly (no bad accept, no
# false reject) and that exactly one verdict is produced per frame (no wedge).
import random
import cocotb
...
def should_accept(w):
    # Golden model: mirrors the RTL's acceptance rules exactly.
    if len(w) < 42:                              return False
    if w[0:6] != MY_MAC_B and w[0:6] != BCAST_B: return False
    if (w[12] << 8 | w[13]) != 0x0800:           return False   # EtherType IPv4
    if w[14] != 0x45:                            return False   # version 4, IHL 5
    if (w[20] & 0x3F) or w[21]:                  return False   # no fragments
    if w[23] != 17:                              return False   # protocol UDP
    s = sum((w[i] << 8) | w[i + 1] for i in range(14, 34, 2))   # IP checksum
    while s >> 16: s = (s & 0xFFFF) + (s >> 16)
    if s != 0xFFFF:                              return False
    if (w[36] << 8 | w[37]) != LISTEN:           return False   # dest port
    if (w[38] << 8 | w[39]) < 8:                 return False   # UDP length sane
    return True
...
def make_case(rng):
    kind = rng.random()
    if kind < 0.35:                        # pristine valid frame
        return bytes(valid_frame())
    if kind < 0.75:                        # valid frame, then corrupt header bytes
        w = valid_frame()
        for _ in range(rng.randint(1, 4)):
            w[rng.randrange(0, 42)] = rng.randrange(0, 256)
        return bytes(w)
    if kind < 0.9:                         # random garbage of legal-ish length
        return bytes(rng.randrange(0, 256) for _ in range(rng.randint(42, 60)))
    w = valid_frame()                      # flip just the dest port
    w[37] ^= 0xFF
    return bytes(w)
...
async def fuzz_matches_reference(dut):
    ...
    for n in range(300):
        ...
        assert len(got) == 1, f"case {n}: expected exactly 1 verdict, got {got} (wedge/dup)"
